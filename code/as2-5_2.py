...
def calculate_derating_metrics(user_text, derating_target=0.80):
    ...
    for line in lines:
        ...
        divider_group = None
        divider_role = None
        div_match = re.search(r'_(DIV\w*)_(TOP|BOT)$', clean_spec)
        if div_match:
    ...
    for group_name, roles in divider_groups.items():
        ...
        if not top or not bot:
            # 配對不完整（該組只找到一顆），退回原本單顆電阻算法，並加上提醒
            for comp in roles.values():
                v = comp["voltage_used"]
                r = comp["val_num"]
                p_max = comp["p_max"]
                ...
                results.append({
                    ...
                    "is_jumper": is_jumper,
                    "is_divider": False,
                    "divider_warning": f"分壓群組 {group_name} 缺少配對電阻（需同時有 _TOP 與 _BOT），已改用單顆電阻算法",
                })
            continue
        ...
        i_divider = vin / total_r     # 分壓迴路的實際電流
        vout = i_divider * r_bot      # 分壓後的節點電壓 (下橋電阻上的壓降)

        for role_name, comp in (("TOP", top), ("BOT", bot)):
            r = comp["val_num"]
            p_max = comp["p_max"]
            is_jumper = comp["is_jumper"]
